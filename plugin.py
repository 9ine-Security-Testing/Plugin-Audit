# Dependencies divided by categories
java_dependencies = [
...
# Rewriting to files
java_dependencies_path = '/mnt/data/pom.xml'
node_dependencies_path = '/mnt/data/package.json'
python_dependencies_path = '/mnt/data/requirements.txt'
dotnet_dependencies_path = '/mnt/data/packages.config'

# Writing Java dependencies (pom.xml format)
with open(java_dependencies_path, 'w') as java_file:
    java_file.write("<dependencies>\n")
    for dep in java_dependencies:
        name, version = dep.split(':')
        java_file.write(f"  <dependency>\n    <groupId>{name}</groupId>\n    <artifactId>{name}</artifactId>\n    <version>{version}</version>\n  </dependency>\n")
    java_file.write("</dependencies>\n")

# Writing Node.js dependencies (package.json format)
with open(node_dependencies_path, 'w') as node_file:
    node_file.write("{\n  \"dependencies\": {\n")
    for dep in node_dependencies:
        name, version = dep.split(':')
        node_file.write(f"    \"{name}\": \"{version}\",\n")
    node_file.write("  }\n}\n")

# Writing Python dependencies (requirements.txt format)
with open(python_dependencies_path, 'w') as python_file:
    python_file.write("\n".join(python_dependencies))

# Writing .NET dependencies (packages.config format)
with open(dotnet_dependencies_path, 'w') as dotnet_file:
    dotnet_file.write("<packages>\n")
    for dep in dotnet_dependencies:
        name, version = dep.split(':')
        dotnet_file.write(f"  <package id=\"{name}\" version=\"{version}\" />\n")
    dotnet_file.write("</packages>\n")

# Returning file paths for review
java_dependencies_path, node_dependencies_path, python_dependencies_path, dotnet_dependencies_path
